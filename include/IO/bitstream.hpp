...
class InBitStream {
 ...
 public:
  ...
  InBitStream& operator=(InBitStream const&) = delete;
...
class OutBitStream {
 ...
 public:
  ...
  void write_char(uint8_t const c) {
    if (n_ == 0) {
  ...
  void write_int(uint32_t const x) {
    for (uint8_t i = 0; i < 4; i++) {
  ...
  void write_long(uint64_t const x) {
