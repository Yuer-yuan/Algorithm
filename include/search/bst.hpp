...
class BST {
 ...
 public:
	...
	size_t
	size(Key const& lo, Key const& hi) const {
		if (cmp(lo, hi) > 0) {
			throw std::runtime_error(
			    "Error getting size between an abnormal range\n");
		}
		if (!get(hi).has_value()) { return rank(hi) - rank(lo); }
		return rank(hi) - rank(lo) + 1;
	};

	// put key-value pair into the tree
	void
	put(Key const& key, Val const& val) {
	...
	OptKey
	select(int const rank) const {
		if (rank < 0 || rank > size()) { return std::nullopt; }
	...
	size_t
	rank(Key const& key) const {
		return rank(root_.get(), key);
	};
	...
	keys() const {
		VecKey foo;
		if (root_ == nullptr) { return foo; }
		return keys(min().value(), max().value());
	}

	// keys between `lo` and `hi`
	VecKey
	keys(Key const& lo, Key const& hi) const {
		VecKey vk;
		if (root_ != nullptr) { keys(root_.get(), vk, lo, hi); }
		return vk;
	}
	...
	// breath first search
	VecKey
	keys_level_order() const {
		VecKey vk;
		if (root_ == nullptr) { return vk; }
		std::queue<Node*> qnode;
		qnode.push(root_);
		while (!qnode.empty()) {
			Node* x = qnode.pop();
			vk.push_back(x->key_);
			if (x->left_ != nullptr) { qnode.push(x->left_.get()); }
			if (x->right_ != nullptr) { qnode.push(x->right_.get()); }
		}
	};

	// print in a level form
	std::ostream&
	operator<<(std::ostream& ostream) const {
 ...
 private:
	...
	UniPtr
	put(UniPtr x, Key const& key, Val const& val) {
		if (x == nullptr) { return std::make_unique<Node>(key, val); }
	...
	Node*
	get(Node* x, Key const& key) const {
		if (x == nullptr) { return nullptr; }
		int const c = cmp(key, x->key_);
		if (c < 0) { return get(x->left_.get(), key); }
		if (c > 0) { return get(x->right_.get(), key); }
		return x;
	}

	UniPtr
	del(UniPtr x, Key const& key) {
		if (x == nullptr) { return nullptr; }
		int const c = cmp(key, x->key_);
		if (c < 0) {
			...
			if (x->left_ == nullptr) { return std::move(x->right_); }
			if (x->right_ == nullptr) { return std::move(x->left_); }
	...
	Node*
	floor(Node* x, Key const& key) const {
		if (x == nullptr) { return nullptr; }
		int const c = cmp(key, x->key_);
		if (c == 0) {
			// equal
	...
	Node*
	ceiling(Node* x, Key const& key) const {
		if (x == nullptr) { return nullptr; }
		int const c = cmp(key, x->key_);
		if (c == 0) { return x; }
		if (c > 0) { return ceiling(x->right_.get(), key); }
	...
	Node*
	select(Node* x, int const rank) const {
		if (x == nullptr) { return nullptr; }
		int const left_size = size(x->left_.get());
		if (rank < left_size) { return select(x->left_.get(), rank); }
		if (rank == left_size) { return x; }
		// rank > left_size
		return select(x->right_.get(), rank - left_size - 1);
	}

	int
	rank(Node* x, Key const& key) const {
	...
	void
	keys(Node* x, VecKey& vk, Key const& lo, Key const& hi) const {
		if (x == nullptr) { return; }
		int const cl = cmp(lo, x->key_), ch = cmp(hi, x->key_);
		if (cl < 0) { keys(x->left_.get(), vk, lo, hi); }
		if (cl <= 0 && ch >= 0) { vk.push_back(x->key_); }
		if (ch > 0) { keys(x->right_.get(), vk, lo, hi); }
	}
	...
	void
	print_node(std::ostream& ostream,
	           size_t const  level,
	           Node* const   x,
	           Direction     d) const {
		if (x == nullptr) {
		...
		for (size_t i = 0; i < level; i++) { ostream << ' '; }
		ostream << "-level " << level << ", size " << x->size_ << ", (" << x->key_
		        << "->" << x->val_ << ") ";
		switch (d) {
			case Direction::Root: ostream << "root"; break;
			case Direction::Left: ostream << "left"; break;
			case Direction::Right: ostream << "right"; break;
		}
		ostream << '\n';
		if (x->left_ != nullptr) {
