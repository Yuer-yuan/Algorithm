...
class BST {
 ...
 public:
  ...
  void put(Key const& key, Val const& val) { put(root_.get(), key, val); }
 ...
 private:
  ...
  void put(Node* x, Key const& key, Val const& val) {
    // `root_` is `nullptr`, create new `root_`
    if (root_ == nullptr) {
    ...
    // not gonna happen
    if (x == nullptr) {
      std::cerr << "Err putting a node to an empty tree\n";
      return;
    }
    if (key < x->key_) {
      // less, put to `left_`
  ...
  Node* select(Node* x, int const rank) {
    if (x == nullptr) {
      return nullptr;
    }
