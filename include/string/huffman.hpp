...
class Huffman {
 ...
 public:
	...
	compress() {
		...
		uint8_t              c;
		while (std::cin.read((char*) &c, sizeof(uint8_t))) { s.push_back(c); }

		// count frequency
		uint64_t const          n = s.size();
		std::array<uint64_t, R> freq_tb;
		for (uint64_t i = 0; i < n; i++) { freq_tb[s[i]] += 1; }
		...
		// encode the message
		for (auto const& c : s) {
			for (auto const& b : st[c]) { out_bitstream.write_bit(b); }
		}
		out_bitstream.flush();
	}
 ...
 private:
   ...
   */
	static ShrPtr
	build_trie(std::array<uint64_t, R> const& freq_table) {
		alg::PriorityQueue<Node, Order<Node>::less> pq;
		for (uint64_t c = 0; c < R; c++) {
			size_t const freq = freq_table[c];
			if (freq > 0) { pq.push(Node((uint8_t) c, freq, nullptr, nullptr)); }
		}
   ...
   */
	static std::array<std::vector<bool>, R>
	build_code(Node* root) {
	...
	static void
	build_code(std::array<std::vector<bool>, R>& st,
	           Node*                             x,
	...
	template<typename T>
	static std::vector<T>
	append(std::vector<T> const& v, T const& e) {
		std::vector<T> v1(v);
		v1.push_back(e);
		return v1;
	}
